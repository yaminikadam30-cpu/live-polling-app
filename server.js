...
const scoreFor=(s,at)=>{const q=activeSlide(s);if(!q?.correctOptionId)return 0;if(!q.duration)return 1000;const elapsed=Math.max(0,Math.min(q.duration*1000,at-s.slideStartedAt));return Math.max(500,Math.round(1000-(500*elapsed/(q.duration*1000))))};
